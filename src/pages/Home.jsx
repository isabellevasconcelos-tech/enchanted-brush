...
export default function Home() {
  ...
  // After OAuth: if logged in but profile incomplete, go to /perfil
  useEffect(() => {
  ...
  return (
    <div className="animate-fade-in">
      <section className="relative text-center py-12 sm:py-20 overflow-hidden">
        ...
        <div className="mt-4"><HiddenStar id="home-hero" /></div>
      </section>
          ...
          {waxSeals.map((seal, i) => {
            const inner = (
              <div className="flex flex-col items-center gap-2">
                <div className="btn-wax-seal group relative">
                  {/* Ícone central */}
                  <span className="relative z-10 transition-transform duration-300 group-hover:scale-110">
                    {seal.emoji ? <span className="text-2xl sm:text-3xl">{seal.emoji}</span> : seal.icon}
                  </span>
                  {/* Brilho dourado no centro */}
                  <div className="absolute inset-0 rounded-full bg-[radial-gradient(circle_at_40%_35%,rgba(212,175,55,0.15),transparent_60%)] pointer-events-none" />
                  {/* Sparkle */}
                  <span className="brand-sparkle absolute" style={{ top: '-4px', right: '8px', '--s-color': '#D4AF37', '--s-size': '2px', '--s-speed': '2s', '--s-delay': `${i * 0.4}s` }} />
                  <span className="brand-sparkle absolute" style={{ bottom: '2px', left: '10px', '--s-color': '#FFD700', '--s-size': '1.5px', '--s-speed': '1.8s', '--s-delay': `${i * 0.4 + 0.8}s` }} />
                </div>
                <span className="font-heading text-xs sm:text-sm text-gold-accent uppercase tracking-[0.2em] mt-1">
                  {seal.title}
                </span>
                <span className="text-enchanted-muted text-[10px] sm:text-xs">
                  {seal.desc}
                </span>
              </div>
            )
            return seal.external ? (
              <a key={seal.to} href={seal.to} target="_blank" rel="noopener noreferrer" className="no-underline">
                {inner}
              </a>
            ) : (
              <Link key={seal.to} to={seal.to} className="no-underline">
                {inner}
              </Link>
            )
          })}
      ...
      <section className="py-10">
        ...
        <Link
          to="/my-story"
          className="home-card group block max-w-lg mx-auto mb-4 sm:mb-5 rounded-2xl overflow-hidden no-underline relative border-2 border-gold-accent/40 shadow-[0_0_20px_rgba(212,175,55,0.12),0_0_40px_rgba(139,26,43,0.1)] transition-all duration-500 hover:border-gold-accent/70 hover:shadow-[0_0_35px_rgba(212,175,55,0.25),0_0_60px_rgba(139,26,43,0.15)] active:scale-[0.98]"
        >
          {/* Background gradient */}
          <div className="absolute inset-0 bg-gradient-to-br from-rose-pastel/30 via-cream-50 to-gold-pastel/20 z-0" />
          <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_left,rgba(139,26,43,0.08),transparent_60%)] z-0" />

          {/* Barra bordo no topo */}
          <div className="h-1.5 w-full bg-gradient-to-r from-transparent via-gold-accent to-transparent z-10 relative" />

          <div className="flex items-center gap-4 p-5 sm:p-6 relative z-10">
            {/* Ícone ✍️ */}
            <span className="home-card-icon w-16 h-16 sm:w-18 sm:h-18 flex-shrink-0 flex items-center justify-center rounded-full bg-rose-accent/15 border border-rose-accent/40 text-rose-accent shadow-[0_0_12px_rgba(139,26,43,0.25)] transition-all duration-500 group-hover:bg-rose-accent/25 group-hover:scale-110 group-hover:shadow-[0_0_20px_rgba(139,26,43,0.4)]">
              <span className="text-3xl">✍️</span>
            </span>

            {/* Texto */}
            <div className="flex-1 min-w-0">
              <span className="font-heading text-sm sm:text-base text-enchanted uppercase tracking-widest leading-tight block">
                Nossa História
              </span>
              <span className="text-enchanted-muted text-xs sm:text-sm leading-relaxed block mt-1">
                A jornada por trás de cada pincelada
              </span>
            </div>

            {/* Seta animada */}
            <div className="flex-shrink-0 w-8 h-8 rounded-full bg-rose-accent/10 border border-rose-accent/30 flex items-center justify-center group-hover:bg-rose-accent/20 group-hover:border-rose-accent/50 transition-all duration-300">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="text-rose-accent group-hover:translate-x-0.5 transition-transform duration-300">
          ...
          {/* Barra inferior */}
          <div className="h-0.5 w-full bg-gradient-to-r from-transparent via-rose-accent/40 to-transparent z-10 relative" />

          {/* Sparkles */}
          <span className="brand-sparkle absolute" style={{ top: '6px', right: '12px', '--s-color': '#D4AF37', '--s-size': '2.5px', '--s-speed': '2s', '--s-delay': '0s' }} />
          <span className="brand-sparkle absolute" style={{ top: '10px', left: '30%', '--s-color': '#FFD700', '--s-size': '2px', '--s-speed': '1.6s', '--s-delay': '0.7s' }} />
          <span className="brand-sparkle absolute" style={{ bottom: '8px', left: '12px', '--s-color': '#C0C0C0', '--s-size': '2px', '--s-speed': '1.8s', '--s-delay': '1s' }} />
          <span className="brand-sparkle absolute" style={{ bottom: '10px', right: '25%', '--s-color': '#D4AF37', '--s-size': '2px', '--s-speed': '2.2s', '--s-delay': '0.4s' }} />
        </Link>

        <div className="grid grid-cols-2 gap-y-8 gap-x-4 max-w-sm mx-auto justify-items-center">
          {menuItems.map((item, i) => (
            <Link key={item.to} to={item.to} className="no-underline">
              <div className="flex flex-col items-center gap-2 w-[130px]">
                <div className="btn-wax-seal group relative">
                  <span className="text-2xl sm:text-3xl relative z-10 transition-transform duration-300 group-hover:scale-110">
                    {item.emoji}
                  </span>
                  <div className="absolute inset-0 rounded-full bg-[radial-gradient(circle_at_40%_35%,rgba(212,175,55,0.15),transparent_60%)] pointer-events-none" />
                  <span className="brand-sparkle absolute" style={{ top: '-4px', right: '8px', '--s-color': '#D4AF37', '--s-size': '2px', '--s-speed': '2s', '--s-delay': `${i * 0.3}s` }} />
                  <span className="brand-sparkle absolute" style={{ bottom: '2px', left: '10px', '--s-color': '#FFD700', '--s-size': '1.5px', '--s-speed': '1.8s', '--s-delay': `${i * 0.3 + 0.6}s` }} />
                </div>
                <span className="font-heading text-xs sm:text-sm text-gold-accent uppercase tracking-[0.2em] mt-1 text-center">
                  {item.title}
                </span>
                <span className="text-enchanted-muted text-[10px] sm:text-xs text-center">
                  {item.desc}
                </span>
              </div>
            </Link>
